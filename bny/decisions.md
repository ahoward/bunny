...
| 2026-03-27 | Gemini eval fixed: `-p` flag + shell redirect + JSON extractor | Gemini CLI needs `-p` for headless mode, `Bun.file()` stdin ignored by gemini. Shell redirect works. Brace-matching JSON extractor handles markdown fences. Previous baseline was inflated (gemini=0 averaged up). |
| 2026-03-27 | QA baseline rebased: 2.47 → 1.68 (honest dual-reviewer scores) | Previous scores were claude-only (gemini=0). With both reviewers scoring, composites are lower but more accurate. Defect counts higher (gemini finds different bugs than claude). |
| 2026-03-28 | Never fire and forget: stderr relay on every non-zero exit | `spawn_sync`/`spawn_async` auto-write failure detail to stderr. QA captures hop stdout/stderr/exit to log files. `dev/bg` splits stdout/stderr, appends tail on failure. No more silent failures anywhere. |
