...
export async function call(path: string, params: Params = null, emit?: Emit): Promise<Result> {
  ...
  if (!handler) {
    const not_found: Result = {
      status:  "error",
      result:  null,
      errors:  { path: [{ code: "not_found", message: `handler not found: ${path}` }] },
      meta:    build_meta()
    }
  ...
  try {
    ...
    const caught: Result = {
      status:  "error",
      result:  null,
      errors:  { handler: [{ code: "exception", message: err instanceof Error ? err.message : String(err) }] },
      meta:    build_meta()
    }
...
export function parse_params(input: string): Result<Params> {
  ...
  if (trimmed === "") {
    return { status: "success", result: null, errors: null, meta: {} as Meta }
  }
  try {
    return { status: "success", result: JSON.parse(trimmed), errors: null, meta: {} as Meta }
  } catch (err) {
    return error({ params: [invalid_json(err instanceof Error ? err.message : "invalid JSON")] })
  }
}

export const app = { call, register, paths, parse_params }
export default app
