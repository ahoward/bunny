...
| 2026-03-02 | Stateful map — persistent SQLite index for bny map | bun:sqlite FTS5 index at .bny/map/index.sqlite; write-through on every `bny map` run; subcommands: query (FTS), focus (AI-focused context), stats; zero new deps; inspired by amdb (which failed on TS) |
| 2026-03-03 | Mobius Development — aspirational README rewrite + roadmap | README frames bny around "Mobius Development" (knowledge → code → knowledge); roadmap refocused on M1-M7 vision items (build, spike, decouple branches, auto-ruminate, loop→propose) |
| 2026-03-03 | bny build + bny spike — dark factory collapse | `bny build` runs full pipeline (specify→plan→tasks→review→implement→ruminate) or single step via subcommand; `bny spike` is same interface guardrails-off (no review, auto-ruminate); brutal CLI consistency |
| 2026-03-03 | Decouple specify from git branches | specify.ts no longer runs `git checkout -b` or `gh issue create`; .bny/current-feature state file tracks active feature; current_feature() checks env → state file → git branch; branches are manual at proposal level |
| 2026-03-03 | Loop→propose pipeline | `bny brane loop --propose [N]` generates proposals after loop completes; delegates to proposal_main() with the loop goal as topic; connects the think→propose arc in the mobius cycle |
| 2026-03-03 | pov→lens rename | More accurate and abstract than "point of view"; lenses filter what gets absorbed; migration shim in load_state() maps active_povs→active_lenses |
| 2026-03-03 | brane eat→digest (top-level), brane digest→brane rebuild | `bny digest` is the professional top-level ingestion command with file:// URI support; `bny brane rebuild` replaces `bny brane digest` for worldview reconstruction; brane/eat kept as hidden alias |
| 2026-03-03 | Smart init — project type detection | detect_project_type() sniffs for bun.lock, package.json, Cargo.toml, go.mod, pyproject.toml, Makefile; generates appropriate dev scripts; pre/post flight gate through ./dev/test |
| 2026-03-03 | README rewrite — knowledge graph framing | README emphasizes persistent knowledge graph, provenance-tracked sources, filterable lenses; "digest" as primary ingestion verb |
| 2026-03-03 | Hardening batch — 8 items P0-P2 | Shell injection fix (array spawn), --model validation (alphanumeric only), SSRF guard (private IP blocklist + max-filesize), parse_json fix (balanced brace extraction), call_claude_json<T>() helper, timeout wrapper, PID-unique temp files, confirm_intake safe default |
| 2026-03-04 | Hardening round 2 — merged Claude+Gemini review, 21 items | confirm() safe default (next/build), SSRF redirect+numeric IP hardening, feature_paths traversal guard, ralph backoff, extract_json_block escape fix, call_claude_with_tools timeout, parse_args early return, timeout=0 handling, map.ts extractor sandbox, find_root() warning, spin --log async, load_state warning, read_dir depth limit, proposal append, require→import |
