  ...
  switch (round) {
    ...
    case 2:
      body = [
        ...
        "",
        "Before writing tests, you MUST perform the following analysis. Include it in your reasoning field.",
        "",
        "### CODE PATH ENUMERATION (do this first)",
        "",
        "Read the source code and enumerate every code path:",
        "",
        "```",
        "| Path | File:Line | Condition | Tested by contract test? |",
        "|------|-----------|-----------|--------------------------|",
        "```",
        "",
        "### INVARIANT DERIVATION",
        "",
        "For each invariant you plan to test:",
        "",
        "```",
        "INVARIANT I[N]: [formal statement, e.g., parse(format(x)) === x]",
        "  DOMAIN PREMISE: [why this must hold — cite spec or domain rules]",
        "  CODE EVIDENCE:  [file:line where this should hold]",
        "  CONTRACT BLIND SPOT: [why existing contract tests don't catch violations]",
        "  ADVERSARIAL INPUT:   [specific input class that could break it]",
        "```",
        "",
        "### Then generate tests targeting these invariants.",
        "",
    ...
    case 3:
      body = [
        ...
        "",
        "The implementation passes contract and property tests. Now write boundary + golden tests.",
        "",
        "Before writing tests, you MUST perform the following analysis. Include it in your reasoning field.",
        "",
        "### COVERAGE GAP ANALYSIS (do this first)",
        "",
        "Read existing tests AND source code. Find what's NOT covered:",
        "",
        "```",
        "GAP G[N]: [file:line] — [code path or condition]",
        "  EXISTING COVERAGE: [which tests touch this area, if any]",
        "  WHY STILL UNTESTED: [what specific input/state is not exercised]",
        "  RISK:               [what bug could hide here — be specific]",
        "  BOUNDARY INPUT:     [exact value that would trigger the gap]",
        "```",
        "",
        "### FRAGILITY MAP",
        "",
        "Identify the weakest spots in the implementation:",
        "",
        "```",
        "FRAGILE F[N]: [file:line] — [why this code is fragile]",
        "  PARSER/VALIDATOR: [what input format does it expect]",
        "  BREAKS WHEN:      [specific malformed input]",
        "  GOLDEN OUTPUT:    [what correct behavior looks like for key inputs]",
        "```",
        "",
        "### Then generate tests targeting these gaps.",
        "",
        "### Boundary Tests",
